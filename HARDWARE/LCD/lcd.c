...
extern u8 AlarmClock[3][3];//闹钟，初步规定最多可设置3个闹钟精确到分,第3位为选中闹钟标志位
extern u8 Clock_Num;//闹钟数量，目前设置最多可设置3个闹钟
extern float mq135_ppm;//气体质量值
...
char light_v[20],mq_v[20],week[20];//date:年/月/日 clock:时/分/秒
extern u8 clockclear[4];//设置闹钟被选中/取消/删除后的清除特定区域的标志位，1代表点击，0代表每点击；
//clockclear[4] 第1~3位为点击了闹钟选中按钮的标志位，第4位为点击了删除的标志位
u8 musicclear[2]={0};//音乐控制界面清除标志位;第1位：标志音量条的清除标志 第2位：标志播放/暂停清除标志
extern Results results[];//天气数据结构体
char weather_text[8];//用来存储天气信息文本
extern char last_nowweather[20];//存储上一次的最新天气情况
extern char last_dailyweather[3][20];//存储上一次的天气预测情况
extern u8 changeflag;//页面切换清屏标志位
u8 wififlag1=0;//wifiesp8266连接标志
u8 wififlag2=0;//wifiesp32连接标志
void LCD_ShowPage(int page)
{
	...
	if(page>=0 && page<=6)
	{
		...
		LCD_ShowxNum(200,0,calendar.hour,2,16,0x00);				//显示时
		LCD_ShowString(216,0,8,16,16,(u8*)":");//日期
		LCD_ShowxNum(224,0,calendar.min,2,16,0x80);				//显示分
		sprintf(date,"%d/%d/%d",calendar.w_year,calendar.w_month,calendar.w_date);
		LCD_ShowString(0,0,80,16,16,(u8*)date);//日期
		switch(calendar.week)
		{
			case 0:
				{
					LCD_ShowString(92,0,72,16,16,(u8*)"Sunday");//星期
				}
				break;
			case 1:
				{
					LCD_ShowString(92,0,72,16,16,(u8*)"Monday");//星期
				}
				break;
			case 2:
				{
					LCD_ShowString(92,0,72,16,16,(u8*)"Tuesday");//星期
				}
				break;
			case 3:
				{
					LCD_ShowString(92,0,72,16,16,(u8*)"Wednesday");//星期
				}
				break;
			case 4:
				{
					LCD_ShowString(92,0,72,16,16,(u8*)"Thursday");//星期
				}
				break;
			case 5:
				{
					LCD_ShowString(92,0,72,16,16,(u8*)"Friday");//星期
				}
				break;
			case 6:
				{
					LCD_ShowString(92,0,72,16,16,(u8*)"Saturday");//星期
				}
				break;  
		}
	...
	else if(page==4)//模式切换选项页面
	{
		POINT_COLOR=LIGHTBLUE;//设置字体为浅蓝色
		LCD_ShowPicture(56,96,128,128,Mode);
		POINT_COLOR=BLUE;//设置字体为蓝色
		LCD_ShowString(88,232,120,24,24,(u8*)"Mode");
	}
	else if(page==5)//传感器数据监测
	{
		POINT_COLOR=MAGENTA;//设置字体为紫红色
		LCD_ShowPicture(56,96,128,128,envir_data);
		POINT_COLOR=BLUE;//设置字体为蓝色
		LCD_ShowString(88,232,120,24,24,(u8*)"Sensor");
	}
	else if(page==6)//实时天气及预报选项页面
	{
		POINT_COLOR=BLUE;//设置字体为绿色
		LCD_ShowPicture(56,96,128,128,Weather);
		POINT_COLOR=BLUE;//设置字体为蓝色
		LCD_ShowString(88,232,120,24,24,(u8*)"Weather");
	}
	else if(page==7)//进入Clock后的页面
	{
		u8 clocknum=Clock_Num;
		if(clockclear[3]==1)
		{
			clockclear[3]=0;
			for(i=2;i>=clocknum;i--)//根据删除的闹钟个数清除对应数量的闹钟
				LCD_Fill(0,16+i*80,240,88+i*80,WHITE);//删除后需要消除的闹钟记录（清除画点数据）
			//LCD_Fill(0,16+Clock_Num*80,240,88+Clock_Num*80,WHITE);//删除后需要消除的闹钟记录（清除画点数据）
			//本来用上面注释的这个就可以了，但考虑到每次点击屏幕会进入中断1/2次，到时有时候没能正确清除
		}
		for(i=0;i<clocknum;i++)//根据添加的闹钟个数显示对应数量的闹钟
		{
			LCD_ShowxNum(16,40+i*80,AlarmClock[i][0],2,24,0x80);				//显示 数字
			LCD_ShowPicture(16,16+i*80,32,22,Clock_Add);
			LCD_ShowPicture(16,66+i*80,32,22,Clock_Sub);
			
			LCD_ShowString(44,40+i*80,8,24,24,(u8*)":");
			
			LCD_ShowxNum(56,40+i*80,AlarmClock[i][1],2,24,0x80);				//显示 数字
			LCD_ShowPicture(56,16+i*80,32,22,Clock_Add);
			LCD_ShowPicture(56,66+i*80,32,22,Clock_Sub);
			
			if(AlarmClock[i][2])//按钮选中标志位(若被选中)
			{
				if(clockclear[i]==1)
				{
					clockclear[i]=0;
					LCD_Fill(180,28+i*80,228,76+i*80,WHITE);//把前一次的按钮状态清掉
				}
				POINT_COLOR=GREEN;//设置字体为绿色
		...
		POINT_COLOR=RED;//设置字体为红色
		LCD_ShowPicture(120,272,48,48,Sub2);//删除闹钟按键
		LCD_ShowPicture(0,272,48,48,Back);//返回按键
//	  POINT_COLOR=GREEN;//设置字体为绿色
//	  LCD_ShowPicture(192,272,48,48,In);//确认按键
		POINT_COLOR=BLUE;//设置字体为蓝色
		LCD_ShowPicture(184,272,48,48,Add2);//添加闹钟按键
		/*****************************************************/
	}
	else if(page==8)//进入音乐功能后的页面
	{
		if(musicclear[0]==1)
		{
			musicclear[0]=0;
			LCD_Fill(58,26,174,38,WHITE);//音量条清色
		}
		if(musicclear[1]==1)
		{
			musicclear[1]=0;
			LCD_Fill(90,216,150,276,WHITE);//播放/暂停清色
		}
		if(Play_flag==0)
		...
		LCD_DrawRectangle(56,24,176,40);//音量条边框
		LCD_Fill(58,26,54+Music_Voice*4,38,GREEN);//填充单色,模拟音量条
	...
	else if(page==9)//进入Setting后的功能控制页面
	{
...
//	  LCD_ShowPicture(192,272,48,48,In);
	...
	else if(page==11)//进入模式选择后的页面
	{
		//暂时设定4种模式
		//u8 HomeMode自动模式标志位 0：手动模式 1：自动模式 2:离家模式 3：电影模式
		LCD_DrawRectangle(8, 108-32,112, 168-32);		   				//画矩形1
		LCD_DrawRectangle(128, 108-32,232, 168-32);		   				//画矩形2
		LCD_DrawRectangle(8, 168,112, 228);		   				//画矩形3
		LCD_DrawRectangle(128, 168,232, 228);		   				//画矩形4
		if(HomeMode==0)
			LCD_Fill(8+4, 112-32,112-4, 164-32,GREEN);		   				//填充单色
		else
			LCD_Fill(8+4, 112-32,112-4, 164-32,RED);		   				//填充单色
		if(HomeMode==1)
			LCD_Fill(128+4, 112-32,232-4, 164-32,GREEN);		   				//填充单色
		else
			LCD_Fill(128+4, 112-32,232-4, 164-32,RED);		   				//填充单色
		if(HomeMode==2)
			LCD_Fill(8+4, 168+4,112-4, 228-4,GREEN);		   				//填充单色
		else
			LCD_Fill(8+4, 168+4,112-4, 228-4,RED);		   				//填充单色
		if(HomeMode==3)
			LCD_Fill(128+4, 168+4,232-4, 228-4,GREEN);		   				//填充单色
		else
			LCD_Fill(128+4, 168+4,232-4, 228-4,RED);		   				//填充单色
	...
	else if(page==12)//进入天气时钟页面(太空人)
	{
		...
		/***************模拟时间*************/
		LCD_ShowString(8,8,128,24,24,(u8*)"Air:good");//天气情况
		LCD_ShowString(168,12,64,16,16,(u8*)"gaozhou");//地点
		/***********************************************************/
//		LCD_ShowString(136,48,60,24,24,"20:02");//时分24*12
//		LCD_ShowString(204,56,16,16,16,"59");//秒16*8
//		
//		LCD_ShowString(8,228,64,16,16,"Friday");//星期
//		LCD_ShowString(96,228,128,24,24,"2022/3/9");//日期
		if(t!=calendar.sec)
		{
			t=calendar.sec;
//			sprintf(clock,"%2d:%2d",calendar.hour,calendar.min,calendar.sec);
//			LCD_ShowString(136,48,60,24,24,clock);//时间
			LCD_ShowxNum(136,48,calendar.hour,2,24,0x00);				//显示时
			LCD_ShowString(160,48,12,24,24,(u8*)":");//日期
			LCD_ShowxNum(172,48,calendar.min,2,24,0x80);				//显示分
			LCD_ShowxNum(204,56,calendar.sec,2,16,0x80);				//显示秒
			sprintf(date,"%d/%d/%d",calendar.w_year,calendar.w_month,calendar.w_date);
			LCD_ShowString(96,228,128,24,24,(u8*)date);//日期		
			switch(calendar.week)
			{
				case 0:
					{
						LCD_ShowString(8,228,64,16,16,(u8*)"Sunday");//星期
						//OLED_ShowString(32,16,(u8*)"Sunday",16,1);
					}
					break;
				case 1:
					{
						LCD_ShowString(8,228,64,16,16,(u8*)"Monday");//星期
						//OLED_ShowString(32,16,(u8*)"Monday",16,1);
					}
					break;
				case 2:
					{
						LCD_ShowString(8,228,64,16,16,(u8*)"Tuesday");//星期
						//OLED_ShowString(32,16,(u8*)"Tuesday",16,1);
					}
					break;
				case 3:
					{
						LCD_ShowString(8,228,64,16,16,(u8*)"Wednesday");//星期
						//OLED_ShowString(32,16,(u8*)"Wednesday",16,1);
					}
					break;
				case 4:
					{
						LCD_ShowString(8,228,64,16,16,(u8*)"Thursday");//星期
						//OLED_ShowString(32,16,(u8*)"Thursday",16,1);
					}
					break;
				case 5:
					{
						LCD_ShowString(8,228,64,16,16,(u8*)"Friday");//星期
						//OLED_ShowString(32,16,(u8*)"Friday",16,1);
					}
					break;
				case 6:
					{
						LCD_ShowString(8,228,64,16,16,(u8*)"Saturday");//星期
...
//					OLED_ShowChar(72,32,'0',16,1);
		...
		LCD_ShowPicture(0,272,48,48,Back);//返回按键
		POINT_COLOR=GREEN;//设置字体为绿色
		LCD_ShowPicture(8,48,32,32,AQ);//空气质量
		POINT_COLOR=YELLOW;//设置字体为黄色
		LCD_ShowPicture(8,88,32,32,Light_bmp);//关照强度
		POINT_COLOR=RED;//设置字体为红色
		LCD_ShowPicture(8,128,32,32,Temp_bmp);//温度
		POINT_COLOR=BLUE;//设置字体为蓝色
		LCD_ShowPicture(8,168,32,32,Humi_bmp);//湿度
//		for(i=0;i<47;i++)
//		{
//			LCD_ShowPicture(120,80,114,128,skyman[i]);//逐帧显示太空人
//			delay_ms(10);
//			LCD_Fill(120,80,240,208,WHITE);
//		}
	}
	else if(page==13)//天气时钟
	{
		POINT_COLOR=RED;//设置字体为红色
	  LCD_ShowPicture(0,272,48,48,Back);
		POINT_COLOR=BLACK;//设置字体为黑色
		...
		{
			Show_Chinese(16,32,4,16,BLACK,WHITE,0);//茂
			Show_Chinese(32,32,5,16,BLACK,WHITE,0);//名
		}
//		sprintf(weather_text,"weather:%s",results[0].now.text);
//		LCD_ShowString(120,24,sizeof(weather_text)*20,16,16,(u8*)weather_text);
		//LCD_ShowString(28,120,144,16,16,(u8*)results[0].now.text);//要换成中文
		if(!strcmp(results[0].now.text,"Sunny"))
		{
			Show_Chinese(44,120,15,16,BLACK,WHITE,0);//晴
			Show_Chinese(60,120,3,16,BLACK,WHITE,0);//天
		}
		else if(!strcmp(results[0].now.text,"Cloudy"))
		{
			Show_Chinese(44,120,16,16,BLACK,WHITE,0);//多
			Show_Chinese(60,120,17,16,BLACK,WHITE,0);//云
		}
		else if(!strcmp(results[0].now.text,"Overcast"))
		{
			Show_Chinese(44,120,18,16,BLACK,WHITE,0);//阴
			Show_Chinese(60,120,3,16,BLACK,WHITE,0);//天
		}
		else if(!strcmp(results[0].now.text,"Shower"))
		{
			Show_Chinese(44,120,20,16,BLACK,WHITE,0);//阵
			Show_Chinese(60,120,25,16,BLACK,WHITE,0);//雨
		}
		else if(!strcmp(results[0].now.text,"Light rain"))
		{
			Show_Chinese(44,120,21,16,BLACK,WHITE,0);//小
			Show_Chinese(60,120,25,16,BLACK,WHITE,0);//雨
		}
		else if(!strcmp(results[0].now.text,"moderate rain"))
		{
			Show_Chinese(44,120,22,16,BLACK,WHITE,0);//中
			Show_Chinese(60,120,25,16,BLACK,WHITE,0);//雨
		}
		else if(!strcmp(results[0].now.text,"heavy rain"))
		{
			Show_Chinese(44,120,23,16,BLACK,WHITE,0);//大
			Show_Chinese(60,120,25,16,BLACK,WHITE,0);//雨
		}
		else if(!strcmp(results[0].now.text,"thunder shower"))
		{
			Show_Chinese(44,120,19,16,BLACK,WHITE,0);//雷
			Show_Chinese(60,120,20,16,BLACK,WHITE,0);//阵
			Show_Chinese(60,120,25,16,BLACK,WHITE,0);//雨
		}
		else if(!strcmp(results[0].now.text,"torrential rain"))
		{
			Show_Chinese(44,120,24,16,BLACK,WHITE,0);//暴
			Show_Chinese(60,120,25,16,BLACK,WHITE,0);//雨
		}
		else
		{
			Show_Chinese(44,120,26,16,BLACK,WHITE,0);//查
			Show_Chinese(60,120,27,16,BLACK,WHITE,0);//询
			Show_Chinese(76,120,28,16,BLACK,WHITE,0);//中
		}
		
		Show_Chinese(120,48,6,16,BLACK,WHITE,0);//温
		Show_Chinese(136,48,8,16,BLACK,WHITE,0);//度
		sprintf(weather_text,": %s C",results[0].now.temperature);
		LCD_ShowString(152,48,sizeof(weather_text)*8,16,16,(u8*)weather_text);
		
		Show_Chinese(120,72,7,16,BLACK,WHITE,0);//湿
		Show_Chinese(136,72,8,16,BLACK,WHITE,0);//度
		sprintf(weather_text,": %s%%",results[0].daily[0].humidity);
		LCD_ShowString(152,72,sizeof(weather_text)*8,16,16,(u8*)weather_text);
		
		Show_Chinese(120,96,9,16,BLACK,WHITE,0);//风
		Show_Chinese(136,96,10,16,BLACK,WHITE,0);//速
		...
		LCD_DrawLine(0, 144,240,144);							//分割线
		//今天明天后天
		//LCD_ShowString(12,160,64,16,16,(u8*)"today");
		Show_Chinese(28,160,0,16,BLACK,WHITE,0);//今
		Show_Chinese(28+16,160,3,16,BLACK,WHITE,0);//天
		//LCD_ShowString(12+76,160,64,16,16,(u8*)"tomorrow");
		Show_Chinese(104,160,1,16,BLACK,WHITE,0);//明
		Show_Chinese(104+16,160,3,16,BLACK,WHITE,0);//天
		//LCD_ShowString(12+76*2,160,64,16,16,(u8*)"houtian");
		Show_Chinese(180,160,2,16,BLACK,WHITE,0);//后
		Show_Chinese(180+16,160,3,16,BLACK,WHITE,0);//天
		
		
		if(results[0].now.text != NULL)//如果获取到天气信息
		{
			if(strcmp(results[0].now.text,last_nowweather))//如果与上次的天气不用，则刷新一次屏幕
			{
				strcpy(last_nowweather,results[0].now.text);
				changeflag=1;
			}
			POINT_COLOR=BLUE;//设置字体为黄色
			if(!strcmp(results[0].now.text,"Sunny"))
			{
				POINT_COLOR=YELLOW;//设置字体为黄色
				LCD_ShowPicture(32,48,64,64,sunny_bmp);//晴
			}
			else if(!strcmp(results[0].now.text,"Cloudy"))
			{
				POINT_COLOR=GRAY;//设置字体为黄色
				LCD_ShowPicture(32,48,64,64,cloudy_bmp);//多云
			}
			else if(!strcmp(results[0].now.text,"Overcast"))
			{
				POINT_COLOR=GRAY;//设置字体为黄色
				LCD_ShowPicture(32,48,64,64,overcast_bmp);//阴
			}
			else if(!strcmp(results[0].now.text,"Shower"))
				LCD_ShowPicture(32,48,64,64,shower_bmp);//阵雨
			else if(!strcmp(results[0].now.text,"Light rain"))
				LCD_ShowPicture(32,48,64,64,Light_rain_bmp);//小雨
			else if(!strcmp(results[0].now.text,"moderate rain"))
				LCD_ShowPicture(32,48,64,64,Moderate_rain_bmp);//中雨
			else if(!strcmp(results[0].now.text,"heavy rain"))
				LCD_ShowPicture(32,48,64,64,Heavy_rain_bmp);//大雨
			else if(!strcmp(results[0].now.text,"thunder shower"))
				LCD_ShowPicture(32,48,64,64,Thunder_shower_bmp);//雷阵雨
			else if(!strcmp(results[0].now.text,"torrential rain"))
				LCD_ShowPicture(32,48,64,64,Torrential_rain_bmp);//暴雨
			else 
				LCD_ShowPicture(32,48,64,64,cloudy_bmp);//晴
			POINT_COLOR=BLUE;//设置字体为蓝色
		}
		if(results[0].daily[0].text_day != NULL || results[0].daily[1].text_day != NULL || results[0].daily[2].text_day != NULL)//如果获取到天气信息
		{
			for(i=0;i<3;i++)
			{
				...
				{
					POINT_COLOR=YELLOW;//设置字体为黄色
					LCD_ShowPicture(12+76*i,208,64,64,sunny_bmp);//晴
				}
				else if(!strcmp(results[0].daily[i].text_day,"Cloudy"))
				{
					POINT_COLOR=GRAY;//设置字体为黄色
					LCD_ShowPicture(12+76*i,208,64,64,cloudy_bmp);//多云
				}
				else if(!strcmp(results[0].daily[i].text_day,"Overcast"))
				{
					POINT_COLOR=GRAY;//设置字体为黄色
					LCD_ShowPicture(12+76*i,208,64,64,overcast_bmp);//阴
				}
				else if(!strcmp(results[0].daily[i].text_day,"Shower"))
					LCD_ShowPicture(12+76*i,208,64,64,shower_bmp);//阵雨
				else if(!strcmp(results[0].daily[i].text_day,"Light rain"))
					LCD_ShowPicture(12+76*i,208,64,64,Light_rain_bmp);//小雨
				else if(!strcmp(results[0].daily[i].text_day,"moderate rain"))
					LCD_ShowPicture(12+76*i,208,64,64,Moderate_rain_bmp);//中雨
				else if(!strcmp(results[0].daily[i].text_day,"heavy rain"))
					LCD_ShowPicture(12+76*i,208,64,64,Heavy_rain_bmp);//大雨
				else if(!strcmp(results[0].daily[i].text_day,"thunder shower"))
					LCD_ShowPicture(12+76*i,208,64,64,Thunder_shower_bmp);//雷阵雨
				else if(!strcmp(results[0].daily[i].text_day,"torrential rain"))
					LCD_ShowPicture(12+76*i,208,64,64,Torrential_rain_bmp);//暴雨
				else 
					LCD_ShowPicture(12+76*i,208,64,64,cloudy_bmp);//晴
			}
		}
	}
}
